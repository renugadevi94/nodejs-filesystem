...
const logEvents = async (currentDateStamp) => {
  ...
  try {
    if (!fs.existsSync(path.join(__dirname, "DATE"))) {
      await fsPromises.mkdir(path.join(__dirname, "DATE"));
    }
    await fsPromises.writeFile(
      path.join(__dirname, "DATE", `${dateTime}.txt`),
      currentDateStamp
    );
    fs.readdir(path.join(__dirname, "DATE"), (err, files) => {
      if (err) {
        console.error(err);
      }
      files.forEach((file) => {
        if (path.extname(file, path.basename(file)) == ".txt") {
          textFolder.push(file);
        }
      });
    });
  } catch (error) {
